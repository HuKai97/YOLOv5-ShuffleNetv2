...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        with Pool(num_threads) as pool:
            ...
            # im_file: 当前这张图片的path相对路径
            # l: 当前这张图片的所有gt框的信息 [gt_num, cls+xywh(normalized)]
            # shape: 当前这张图片的形状 shape
            # segments: 当前这张图片所有的多边形gt信息 [多边形个数, cls+xy1...(normalized)]
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # Apply cutouts 随机进行cutout增强 v5源码作者默认是没用用这个的 感兴趣的可以测试一下
...
def getGtLabelByImagePath(img_path, im0s_shape):
    """
    根据图片地址返回gt label
    img_path: 图片路径
    im0s_shape: (4032, 3024, 3) = (h, w, c)
    """
    new_lines = []
    w, h = im0s_shape[1], im0s_shape[0]
    label_path = img2label_paths(img_path)[0]
    # 打开当前txt文件 读取每一行信息 存放在lines中
    ...
    for line in lines:
        ...
        y = np.copy(coor_list)
        # # line: [c x y x y]
        y[0] = w * (float(coor_list[0]) - float(coor_list[2]) / 2)
        y[1] = h * (float(coor_list[1]) - float(coor_list[3]) / 2)
        y[2] = w * (float(coor_list[0]) + float(coor_list[2]) / 2)  # bottom right x
        y[3] = h * (float(coor_list[1]) + float(coor_list[3]) / 2)  # bottom right y
        new_lines.append(np.append(c, y))
    return new_lines
...
def verify_image_label(args):
    ...
    :return l: 当前这张图片的所有gt框的label信息(不包含segment多边形标签) [gt_num, cls+xywh(normalized)]
    :return shape: 当前这张图片的形状 shape
    :return segments: 当前这张图片所有gt的label信息(包含segment多边形标签) [gt_num, xy1...]
    ...
    try:
        ...
        #         assert f.read() == b'\xff\xd9', 'corrupted JPEG'   # 判定jpg是否包含结束字段 是否完整
        ...
        if os.path.isfile(lb_file):  # 如果这个label路径存在
            ...
            with open(lb_file, 'r') as f:  # 读取label文件
                ...
                # 如果label某一列数大于8, 则认为label是存的segment的polygon点(多边形)  就不是矩阵
...
def load_mosaic(self, index):
    ...
    :return: img4: mosaic和随机透视变换后的一张图片
    ...
    for i, index in enumerate(indices):
        ...
        # labels: 获取对应拼接图像的label信息(不包含segments多边形)
        # segments: 获取对应拼接图像的label信息(包含segments多边形)
    ...
    # a = labels4.shape
    # copy_paste随机粘贴复制增强  https://arxiv.org/abs/1902.07296
    # img4, labels4 = self.copy_paste(img4, labels4)
    # b = labels4.shape
    # if a == b:
    #     print(1)
    # else:
    ...
    # 测试代码 测试copy_paste随机粘贴复制增强效果
    # cv2.imshow("copy_paste", img4)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # print(img4.shape)
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10,
    ...
    """通常会在在load_mosaic中调用 用在mosaic操作之后 + random_perspective操作
    随机透视变换  对mosaic整合后的图片进行随机旋转、缩放、平移、裁剪，透视变换，并resize为输入大小img_size
    :params img: mosaic整合后的图片(img4) [2*img_size, 2*img_size]
    :params targets: mosaic整合后的gt label(labels4)
    :params segments: mosaic整合后的gt segments4(当数据集gt中有多边形gt框 segments4就是数据集所有的gt框label信息
                      如果数据集全部都是矩形gt框 那么segments4为None)
    :params degrees: float
    :params translate: float
    :params scale: float
    :params shear: float
    :params perspective: float
    :params border: [-320, -320]
    """
    # 设定输出图片的 H W
...
def cutout(image, labels):
    ...
    for s in scales:
        ...
        if len(labels) and s > 0.03:
            ...
            labels = labels[ioa < 0.60]  # remove >60% obscured labels  不能切的太大  ioa < 0.60
...
def copy_paste_orign(im, labels, segments, p=0.5):
