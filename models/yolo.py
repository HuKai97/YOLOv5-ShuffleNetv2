...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # 遍历backbone和head的每一层
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d,
                 Focus, CrossConv, BottleneckCSP, C3, C3TR, CBAM, RepVGGBlock,
        ...
        for i in range(self.nl):  # 对三个feature map分别进行处理
            ...
            if not self.training:
                ...
                if self.inplace:
                    ...
                    # 关于AWS推理, 其实和模型算法关系不大 应该是亚马逊的一个部署推理使用, 一般用不太到
                    # 可能和他们框架有冲突 比如onxx导出的数据结构问题  不需要太关注
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):
        ...
        logger.info([x.shape for x in self.forward(torch.zeros(1, ch, 320, 320))])
